<header
    class="sticky top-0 left-0 w-full h-24 -mb-24 bg-gradient-to-b from-black/10 to-transparent flex items-start pt-4 pr-4 justify-end z-50 pointer-events-none">

    {{-- Desktop header --}}
    <div
        class="pointer-events-auto h-16 w-52 bg-[#F5F5F5] p-2 rounded-full items-center justify-between gap-2 shadow-lg hidden lg:flex">
        ...
        <div class="flex items-center gap-2">
            ...
            <div class="flex flex-col">
                <h2 class="text-sm font-semibold text-[#363062]">{{ ucfirst(auth()->user()->nama_lengkap) }}</h2>
        ...
        <div x-data="{ open: false }" class="relative inline-block text-left mr-2">
            ...
            <div x-show="open" x-cloak
                class="absolute right-0 mt-6 w-48 rounded-xl bg-white border border-gray-200 shadow-lg py-1">
                <a href="{{ route('profil-edit') }}"
                    class="block px-4 py-2 text-xs font-medium text-[#363062] hover:bg-gray-50">Profil Saya</a>
                <div class="border-t border-gray-100 my-1"></div>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button
                        class="block text-left w-full px-4 py-2 text-xs font-bold text-red-500 hover:bg-red-50">Keluar</button>
                </form>
            </div>
        </div>
    </div>
    ...
    {{-- Mobile header --}}
    <div class="lg:hidden fixed top-4 right-4 z-50 pointer-events-auto" x-data="{ open: false }">
        <button @click="open = !open" class="h-14 w-14 rounded-full overflow-hidden shadow-lg ring-2 ring-white">
            <img src="{{ auth()->user()->foto_url }}" class="w-full h-full object-cover">
        </button>
        ...
        <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-150"
            ...
            class="absolute right-0 mt-2 w-52 rounded-2xl bg-white border border-gray-200 shadow-xl py-2">
            ...
            <div class="flex items-center gap-2 px-3 pb-2">
                ...
                <div class="flex flex-col min-w-0">
                    <p class="text-xs font-semibold text-[#363062] truncate">{{ ucfirst(auth()->user()->nama_lengkap) }}
                    </p>
                    <p class="text-xs text-[#4D4C7D] font-light">{{ ucfirst(auth()->user()->role) }}</p>
                </div>
            </div>
