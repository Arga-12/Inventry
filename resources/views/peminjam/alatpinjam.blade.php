...
<div class="mx-auto py-8 px-4">
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-10 items-stretch">
        ...
        <div class="flex flex-col col-span-3">
            <h1 class="text-4xl font-bold">Peminjaman Alat & Barang</h1>
            <p class="text-gray-500">Lakukan peminjaman alat & barang yang telah disediakan oleh Inventry</p>
        </div>

        <div class="h-auto w-full bg-white flex flex-shrink-0 flex-col col-span-3 items-stretch justify-between gap-4 p-6 shadow-lg rounded-[20px]">
            ...
            <div class="flex items-center gap-3">
                ...
                <div x-data="filter()" class="relative">
                    <button @click="open = !open" class="w-11 h-11 flex items-center justify-center border border-[#363062] rounded-full hover:bg-gray-100">
                    ...
                    <div x-show="open" @click.outside="open = false" x-cloak
                        class="absolute right-0 z-20 mt-2 w-72 bg-white rounded-[20px] shadow-xl border border-gray-100">
                        <div class="p-3">
                            <p class="px-3 py-2 text-xs font-bold text-gray-500">Kategori</p>
                            <template x-for="kat in kategoris" :key="kat.id">
                                <label class="flex items-center justify-between px-3 py-2 rounded-xl hover:bg-gray-50 cursor-pointer">
                                    <span class="text-sm text-[#363062]" x-text="kat.nama_kategori"></span>
                                    <div class="w-5 h-5 rounded-full border-2 flex items-center justify-center"
                                        :class="kategoriIds.includes(kat.id) ? 'border-[#363062] bg-[#363062]' : 'border-gray-300'">
                                        <svg x-show="kategoriIds.includes(kat.id)" class="w-3 h-3 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
                                            <path d="M5 13l4 4L19 7"/>
                                        </svg>
                                    </div>
                                    <input type="checkbox" class="hidden" :value="kat.id" x-model="kategoriIds">
                                </label>
                            </template>
                            ...
                            <template x-for="dur in durasiList" :key="dur.value">
                                <label class="flex items-center justify-between px-3 py-2 rounded-xl hover:bg-gray-50 cursor-pointer">
                                    ...
                                    <div class="w-5 h-5 rounded-full border-2 flex items-center justify-center"
                                        :class="durasiIds.includes(dur.value) ? 'border-[#363062] bg-[#363062]' : 'border-gray-300'">
                                        <svg x-show="durasiIds.includes(dur.value)" class="w-3 h-3 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
                                            <path d="M5 13l4 4L19 7"/>
                                        </svg>
                                    </div>
                                    <input type="checkbox" class="hidden" :value="dur.value" x-model="durasiIds">
                                </label>
                            </template>
        ...
        {{-- Card Alat --}}
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 col-span-3">
