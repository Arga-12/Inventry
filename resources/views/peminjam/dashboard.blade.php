...
<div class="mx-auto py-8 px-4">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 items-stretch">
                ...
                <div class="h-max w-full bg-white shadow-lg rounded-[20px] flex flex-col items-start p-6 gap-2">
                    ...
                    <div class="w-full h-2 rounded-full overflow-hidden flex mt-3">
                        @foreach($kategoriStats as $stat)
                            <div class="h-full" style="width: {{ $stat['percentage'] }}%; background-color: {{ $stat['color'] }}"></div>
                        @endforeach
                    </div>
        ...
        <div class="h-full bg-gradient-to-r from-[#363062] to-[#4D4C7D] rounded-[20px] p-6 shadow-lg flex flex-col justify-between">
            <div class="flex justify-between">
                <div class="flex flex-col">
                    ...
                    <p class="text-white text-sm">Quick search pada kolom dibawah</p>
                </div>
        ...
        <div class="relative flex flex-col h-96">
            ...
            {{-- title --}}
            <div class="mb-4">
                <h2 class="text-3xl font-bold">Peminjaman Anda saat ini</h2>
                ...
                <div class="relative w-full h-20 flex-shrink-0 bg-white rounded-[20px] flex items-center px-10 shadow-md">
                    ...
                    <div class="ml-auto flex items-center gap-3">
                        ...
                        <div class="flex items-center gap-1">
                            <span class="text-[10px] text-[#363062] bg-[#363062]/10 p-1.5 rounded-lg">
                                {{ \Carbon\Carbon::parse($pinjam->tanggal_disetujui)->format('H:i | d - m - Y') }}
                            </span>
                            <span class="text-sm text-[#363062]">-</span>
                            <span class="text-[10px] text-[#363062] bg-[#363062]/10 p-1.5 rounded-lg">
                                {{ \Carbon\Carbon::parse($pinjam->deadline)->format('H:i | d - m - Y') }}
                            </span>
                        </div>

                        <a href="{{ route('peminjam-peminjaman') }}" class="h-8 w-24 p-2 rounded-full bg-[#4D4C7D] flex items-center justify-center text-white text-xs font-semibold hover:bg-[#363062] transition-colors">
                            Selengkapnya
                        </a>
                    </div>
                </div>
        ...
        <div class="relative w-full h-96 bg-white rounded-[20px] p-10 overflow-hidden shadow-lg">
            ...
            <div class="relative z-10 flex flex-col h-full text-white">
                <div class="space-y-1">
                    ...
                    <p class="text-lg font-light">Optimalkan alur kerja Anda dengan rekomendasi inventaris
                ...
                <div class="mt-auto flex justify-between items-end gap-10">
                    ...
                    <!-- kanan -->
                    <a href="{{ route('peminjam-alat') }}?alat_id={{ $rekom->id }}" class="h-8 w-48 p-2 rounded-full border border-white flex items-center justify-center text-white text-sm font-semibold hover:bg-white hover:text-[#4D4C7D] transition-colors">
                        Pinjam Sekarang
                    </a>
                </div>
            </div>
