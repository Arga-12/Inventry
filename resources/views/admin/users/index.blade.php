...
<div class="mx-auto py-8 px-4">
		...
		<div class="grid grid-cols-1 md:grid-cols-3 gap-10 items-start">
			...
			<div class="flex flex-col gap-4 items-start">
				...
				<div class="flex items-center gap-2">
					...
					<span class="text-sm text-gray-500">{{ $stats['online']['peminjam'] }} Peminjam online saat ini</span>
				</div>
			</div>
			...
			<div class="flex flex-col gap-4 items-start">
				...
				<div class="flex items-center gap-2 text-sm text-gray-500">
					<div class="w-2 h-2 bg-green-500 rounded-full"></div>
					<span>{{ $stats['online']['petugas'] }} Petugas online saat ini</span>
				</div>
			</div>
			...
			<div class="flex flex-col gap-4 items-start">
				...
				<div class="flex items-center gap-2 text-sm text-gray-500">
					<div class="w-2 h-2 bg-green-500 rounded-full"></div>
					<span>{{ $stats['online']['admin'] }} Admin online saat ini</span>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="lg:col-span-3 w-full">
		<div class="w-full flex flex-col gap-6">
			...
			<form method="GET" action="{{ route('admin-pengguna') }}" class="flex items-center justify-between gap-4 flex-wrap">
				...
				<div class="flex items-center gap-3 flex-wrap">
					...
					<div x-data="{ open: false }" class="relative inline-block text-left">
						<button type="button" @click="open = !open" class="w-11 h-11 flex items-center justify-center border border-[#363062] rounded-full hover:bg-gray-100">
						...
						<div x-show="open" @click.outside="open = false" style="display:none" x-transition.opacity
							...
							<div class="py-2">
								...
								<label class="flex items-center px-4 py-2.5 text-sm text-[#363062] hover:bg-gray-50 cursor-pointer transition">
									<input type="radio" name="role_filter" value="" onchange="this.form.submit()" class="mr-3 w-4 h-4 text-[#363062]" {{ request('role_filter') == '' ? 'checked' : '' }}>
									Semua Role
								</label>

								<label class="flex items-center px-4 py-2.5 text-sm text-[#363062] hover:bg-gray-50 cursor-pointer transition">
									<input type="radio" name="role_filter" value="admin" onchange="this.form.submit()" class="mr-3 w-4 h-4 text-[#363062]" {{ request('role_filter') == 'admin' ? 'checked' : '' }}>
									Admin
								</label>

								<label class="flex items-center px-4 py-2.5 text-sm text-[#363062] hover:bg-gray-50 cursor-pointer transition">
									<input type="radio" name="role_filter" value="petugas" onchange="this.form.submit()" class="mr-3 w-4 h-4 text-[#363062]" {{ request('role_filter') == 'petugas' ? 'checked' : '' }}>
									Petugas
								</label>

								<label class="flex items-center px-4 py-2.5 text-sm text-[#363062] hover:bg-gray-50 cursor-pointer transition">
									<input type="radio" name="role_filter" value="peminjam" onchange="this.form.submit()" class="mr-3 w-4 h-4 text-[#363062]" {{ request('role_filter') == 'peminjam' ? 'checked' : '' }}>
									Peminjam
								</label>
								...
								<label class="flex items-center px-4 py-2.5 text-sm text-[#363062] hover:bg-gray-50 cursor-pointer transition">
									<input type="radio" name="status_filter" value="" onchange="this.form.submit()" class="mr-3 w-4 h-4 text-[#363062]" {{ request('status_filter') == '' ? 'checked' : '' }}>
									Semua Status
								</label>

								<label class="flex items-center px-4 py-2.5 text-sm text-[#363062] hover:bg-gray-50 cursor-pointer transition">
									<input type="radio" name="status_filter" value="aktif" onchange="this.form.submit()" class="mr-3 w-4 h-4 text-[#363062]" {{ request('status_filter') == 'aktif' ? 'checked' : '' }}>
								...
								<label class="flex items-center px-4 py-2.5 text-sm text-[#363062] hover:bg-gray-50 cursor-pointer transition">
									<input type="radio" name="status_filter" value="nonaktif" onchange="this.form.submit()" class="mr-3 w-4 h-4 text-[#363062]" {{ request('status_filter') == 'nonaktif' ? 'checked' : '' }}>
									<span class="flex items-center gap-2">
					...
					<a href="{{ route('admin-pengguna') }}" class="px-4 py-3 text-sm text-[#363062]/90 hover:text-[#363062] border border-[#363062] rounded-full transition font-semibold">
						Reset Filter
					</a>
					@endif
				...
				<div class="flex items-center gap-3 flex-wrap">
					<a href="{{ route('admin-pengguna-create') }}" class="px-4 py-3 flex items-center gap-2 border border-transparent text-white font-semibold rounded-full text-sm bg-[#363062] group hover:bg-transparent hover:text-[#363062] hover:border-[#363062] transition-all">
						Tambahkan User
						...
						<tbody class="text-[#363062]">
							...
							<tr class="border-b border-gray-400 hover:bg-gray-50 transition">
								...
								<td class="py-3 px-4">
									...
									@endphp
									<span class="inline-flex px-3 py-1 text-xs text-white bg-gradient-to-r {{ $roleColor }} rounded-full">
										{{ ucfirst($user->role) }}
									</span>
								</td>
								<td class="py-3 px-4">
									...
									@else
									<span class="inline-flex items-center gap-2 px-3 py-1 text-xs font-medium bg-gray-100 text-gray-500 rounded-full">
										<span class="w-2 h-2 bg-gray-400 rounded-full"></span>
										Offline
									</span>
									@endif
								...
								<td class="py-3 px-4">
									<div class="flex items-center gap-2">
										<a href="{{ route('admin-pengguna-edit', $user->username) }}" class="px-3 py-1 text-sm rounded-full border border-[#363062] text-[#363062] hover:bg-[#363062] hover:text-white transition">
											Edit
										</a>
										<form action="{{ route('admin-pengguna-destroy', $user) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus user ini?')">
											@csrf
											@method('DELETE')
											<button type="submit" class="px-3 py-1 text-sm rounded-full border border-red-400 text-red-500 hover:bg-red-500 hover:text-white transition">
												Hapus
											</button>
										</form>
									</div>
					...
					@if($users->hasPages())
						<div class="flex items-center justify-between mt-6">
							...
							<div class="flex items-center gap-2">
								...
								<button type="button" disabled class="p-3 rounded-full border text-sm flex items-center justify-center opacity-50 cursor-not-allowed">
									<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 rotate-180" viewBox="0 0 12 24">
										<defs><path id="SVG1pzpbdYY" fill="currentColor" d="m7.588 12.43l-1.061 1.06L.748 7.713a.996.996 0 0 1 0-1.413L6.527.52l1.06 1.06l-5.424 5.425z"/></defs>
										<use fill-rule="evenodd" href="#SVG1pzpbdYY" transform="rotate(-180 5.02 9.505)"/>
									</svg>
								</button>
								@else
								<a href="{{ $users->previousPageUrl() }}" class="p-3 rounded-full border text-sm flex items-center justify-center hover:bg-gray-100 transition">
									<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 rotate-180" viewBox="0 0 12 24">
										<defs><path id="SVG1pzpbdYY" fill="currentColor" d="m7.588 12.43l-1.061 1.06L.748 7.713a.996.996 0 0 1 0-1.413L6.527.52l1.06 1.06l-5.424 5.425z"/></defs>
										<use fill-rule="evenodd" href="#SVG1pzpbdYY" transform="rotate(-180 5.02 9.505)"/>
									</svg>
								...
								<div class="flex items-center gap-1 bg-gray-100 p-3 rounded-full">
									@php
										...
										if ($start > 1) {
											echo '<a href="' . $users->url(1) . '" class="px-3 py-1 rounded-full text-sm hover:bg-[#363062] hover:text-white transition">1</a>';
											if ($start > 2) {
										...
										for ($i = $start; $i <= $end; $i++) {
											$activeClass = ($i == $currentPage) ? 'bg-[#363062] text-white' : 'hover:bg-gray-200';
											echo '<a href="' . $users->url($i) . '" class="px-3 py-1 rounded-full text-sm ' . $activeClass . ' transition">' . $i . '</a>';
										}
										...
										if ($end < $lastPage) {
											...
											echo '<a href="' . $users->url($lastPage) . '" class="px-3 py-1 rounded-full text-sm hover:bg-[#363062] hover:text-white transition">' . $lastPage . '</a>';
										}
									@endphp
								</div>
								...
								<a href="{{ $users->nextPageUrl() }}" class="p-3 rounded-full border text-sm flex items-center justify-center hover:bg-gray-100 transition">
									<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 12 24">
										<defs><path id="SVG1pzpbdYY" fill="currentColor" d="m7.588 12.43l-1.061 1.06L.748 7.713a.996.996 0 0 1 0-1.413L6.527.52l1.06 1.06l-5.424 5.425z"/></defs>
										<use fill-rule="evenodd" href="#SVG1pzpbdYY" transform="rotate(-180 5.02 9.505)"/>
									</svg>
								</a>
