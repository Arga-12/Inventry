...
<div class="mx-auto py-8 px-4">
    <div class="w-full flex flex-col gap-10">
        ...
        <form action="{{ route('admin-alat-update', $alat, $kategori) }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-10">
            @csrf
            @method('PUT')
            {{-- FOTO BARANG --}}
            ...
            <div class="flex flex-col gap-6">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <select name="kategori_id" class="w-full border border-gray-500 rounded-xl px-4 py-3 bg-white focus:outline-none">
                        ...
                        <option value="{{ $data->id }}" {{ $alat->kategori_id == $data->id ? 'selected' : '' }}>{{ $data->nama_kategori }}</option>
                        @endforeach
                    </select>
                </div>
