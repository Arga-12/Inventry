	...
	<div class="mx-auto pt-20 pb-8 lg:py-8 px-4">
		<div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
			...
			<div class="lg:col-span-3 flex flex-col gap-1">
				<h1 class="text-2xl sm:text-4xl font-bold">Manajemen Alat & Barang</h1>
			...
			<div class="lg:col-span-3 grid grid-cols-1 lg:grid-cols-2 gap-6">
				<div class="bg-white border border-gray-300 rounded-[20px] p-6 shadow-lg flex flex-col justify-between">
					...
					<div class="flex items-start justify-between">
						<h2 class="text-lg sm:text-xl font-semibold">Total Stok</h2>

						@if(($totalSedangDipinjam ?? 0) > 0)
							<span
								class="px-3 py-1 flex items-center gap-2 bg-[#F99417]/20 rounded-full text-xs sm:text-sm shrink-0 text-[#F99417]">
								<div class="h-2 w-2 rounded-full bg-[#F99417]"></div>
								{{ $totalSedangDipinjam }} items sedang dipinjam
							</span>
						@endif
					</div>

					<div class="text-4xl sm:text-5xl font-bold text-[#363062] mt-4">
						{{ $stoktotal }}
					</div>

					<p class="text-xs sm:text-sm text-gray-500 mt-4">
						@if ($totalstokmenipis != 0)
							Mohon perhatikan untuk <span class="font-semibold text-red-500">{{ $totalstokmenipis }} Alat</span>
							lainnya yang mengalami kekurangan stok!
						@else
				...
				<div class="bg-white border border-gray-300 rounded-[20px] p-6 shadow-lg flex flex-col gap-4 h-full">
					...
					<div class="flex items-center justify-between">
						...
						<a href="{{ route('admin-kategori') }}"
							class="px-3 flex items-center gap-2 py-1 text-xs border border-[#363062] text-[#363062] rounded-full hover:bg-[#363062] hover:text-white transition">
							Selengkapnya
				...
				<div class="bg-white border border-gray-300 rounded-[20px] p-6 shadow-lg">
					<div class="w-full flex items-center justify-between gap-4 flex-wrap mb-6">
						<form action="{{ route('admin-alat') }}" method="GET" class="flex items-center gap-3 flex-wrap">
							<div class="relative flex-1 w-full h-full flex-shrink-0">
								...
								<input type="text" name="search_stok" placeholder="Cari nama alat..."
									value="{{ request('search_stok') }}"
								...
								<div x-show="open" @click.outside="open = false" style="display: none;" x-transition.opacity
									...
									<div class="py-2">
										...
										<label
											...
											<input type="radio" name="kategori_stok" value="" onchange="this.form.submit()"
												class="mr-3 w-4 h-4 text-[#363062] focus:ring-[#363062]" {{ request('kategori_stok') == '' ? 'checked' : '' }}>
											Semua Kategori
										</label>
										...
										@foreach ($kategori as $kat)
											<label
												class="flex items-center px-4 py-2.5 text-sm text-[#363062] hover:bg-gray-50 cursor-pointer transition">
												<input type="radio" name="kategori_stok" value="{{ $kat->id }}"
													onchange="this.form.submit()"
													class="mr-3 w-4 h-4 text-[#363062] focus:ring-[#363062]" {{ request('kategori_stok') == $kat->id ? 'checked' : '' }}>
												{{ $kat->nama_kategori }}
											</label>
										@endforeach
									</div>
					...
					@if($stokmenipis->count() > 0)
						<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
							@foreach ($stokmenipis as $data)
								{{-- card alat stok menipis --}}
								<div
									class="flex h-40 sm:h-36 bg-white border border-gray-200 rounded-[20px] overflow-hidden shadow-sm hover:shadow-lg transition">
									<div class="w-1/3 relative">
										<img src="{{ asset('storage/' . $data->gambar) }}" alt="gambar alat"
											class="absolute inset-0 w-full h-full object-cover bg-center">
										<div class="absolute inset-0 bg-gradient-to-r from-black/30 to-transparent"></div>
									</div>

									<div class="w-2/3 p-3 sm:p-4 flex flex-col justify-between">
										<div class="flex items-start justify-between">
											<div>
												<h3 class="font-semibold text-sm sm:text-base text-[#363062] line-clamp-1">
													{{ $data->nama_alat }}
												</h3>
											...
											<span
												class="text-[10px] sm:text-xs bg-red-100 text-red-600 px-2 py-1 rounded-full whitespace-nowrap shrink-0">
												Stok menipis
											</span>
										</div>
										...
										<div class="flex items-center justify-between">
											<span class="text-xs sm:text-sm font-semibold text-[#363062]">
												Sisa: {{ $data->stok }}
											</span>

											<div class="flex items-center gap-2">
												<a href="{{ route('admin-alat-edit', $data) }}"
													class="text-[10px] sm:text-xs px-2.5 sm:px-3 py-1 border border-[#F99417] text-[#F99417] rounded-full hover:bg-[#F99417] hover:text-white transition">
													Edit
												</a>

												<form action="{{ route('admin-alat-destroy', $data) }}" method="POST"
													class="inline">
													@csrf
													@method('DELETE')
													<button
														class="text-[10px] sm:text-xs px-2.5 sm:px-3 py-1 border border-red-500 text-red-500 rounded-full hover:bg-red-500 hover:text-white transition">
														Hapus
													</button>
												</form>
											</div>
							...
							@if($stokmenipis->hasPages())
								<div class="flex flex-col sm:flex-row items-center justify-between gap-4">
									...
									<div class="text-sm text-gray-500 order-2 sm:order-1 text-center sm:text-left">
										Menampilkan {{ $stokmenipis->firstItem() }} sampai {{ $stokmenipis->lastItem() }} dari
										{{ $stokmenipis->total() }} alat
									</div>
									...
									<div class="flex items-center gap-2 order-1 sm:order-2">
										...
										@if($stokmenipis->onFirstPage())
											<button type="button" disabled
												...
												<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 rotate-180" viewBox="0 0 12 24">
													<defs>
														<path id="stok_prev" fill="currentColor"
															d="m7.588 12.43l-1.061 1.06L.748 7.713a.996.996 0 0 1 0-1.413L6.527.52l1.06 1.06l-5.424 5.425z" />
													</defs>
													<use fill-rule="evenodd" href="#stok_prev" transform="rotate(-180 5.02 9.505)" />
												</svg>
											</button>
										@else
											<a href="{{ $stokmenipis->appends(request()->query())->previousPageUrl() }}"
												class="p-3 rounded-full border bg-white text-sm flex items-center justify-center hover:bg-gray-100 transition shadow-sm">
												<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 rotate-180" viewBox="0 0 12 24">
													<defs>
														<path id="stok_prev2" fill="currentColor"
															d="m7.588 12.43l-1.061 1.06L.748 7.713a.996.996 0 0 1 0-1.413L6.527.52l1.06 1.06l-5.424 5.425z" />
													</defs>
													<use fill-rule="evenodd" href="#stok_prev2" transform="rotate(-180 5.02 9.505)" />
												</svg>
											</a>
										@endif
										...
										<div class="flex items-center gap-1 bg-gray-100 p-1.5 sm:p-2 rounded-full shadow-inner">
											@php
												$currentPage = $stokmenipis->currentPage();
												$lastPage = $stokmenipis->lastPage();
												$start = max(1, $currentPage - 1);
												$end = min($lastPage, $currentPage + 1);

												if ($start > 1) {
													$activeClassFirst = (1 == $currentPage)
														? 'bg-[#363062] text-white'
														: 'hover:bg-gray-200 text-gray-700';

													echo '<a href="' . $stokmenipis->appends(request()->query())->url(1) . '" class="w-8 h-8 flex items-center justify-center rounded-full text-sm font-medium transition ' . $activeClassFirst . '">1</a>';
												...
												for ($i = $start; $i <= $end; $i++) {
													...
													echo '<a href="' . $stokmenipis->appends(request()->query())->url($i) . '" class="w-8 h-8 flex items-center justify-center rounded-full text-sm font-medium transition ' . $activeClass . '">' . $i . '</a>';
												}
												...
												if ($end < $lastPage) {
													...
													$activeClassLast = ($lastPage == $currentPage)
														? 'bg-[#363062] text-white'
														: 'hover:bg-gray-200 text-gray-700';

													echo '<a href="' . $stokmenipis->appends(request()->query())->url($lastPage) . '" class="w-8 h-8 flex items-center justify-center rounded-full text-sm font-medium transition ' . $activeClassLast . '">' . $lastPage . '</a>';
												}
											@endphp
										</div>

										{{-- Tombol Next --}}
										@if($stokmenipis->hasMorePages())
											<a href="{{ $stokmenipis->appends(request()->query())->nextPageUrl() }}"
												class="p-3 rounded-full border bg-white text-sm flex items-center justify-center hover:bg-gray-100 transition shadow-sm">
												<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 12 24">
													<defs>
														<path id="stok_next" fill="currentColor"
															d="m7.588 12.43l-1.061 1.06L.748 7.713a.996.996 0 0 1 0-1.413L6.527.52l1.06 1.06l-5.424 5.425z" />
													</defs>
													<use fill-rule="evenodd" href="#stok_next" transform="rotate(-180 5.02 9.505)" />
												</svg>
											</a>
										@else
											<button type="button" disabled
												...
												<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 12 24">
													<defs>
														<path id="stok_next2" fill="currentColor"
															d="m7.588 12.43l-1.061 1.06L.748 7.713a.996.996 0 0 1 0-1.413L6.527.52l1.06 1.06l-5.424 5.425z" />
													</defs>
													<use fill-rule="evenodd" href="#stok_next2" transform="rotate(-180 5.02 9.505)" />
												</svg>
											</button>
										@endif
					...
					@else
						<div class="w-full flex flex-col items-center gap-3 justify-center py-8">
							<svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 text-gray-500" viewBox="0 0 48 48">
			...
			<div class="lg:col-span-3">
				<div class="w-full flex flex-col gap-6">
					...
					<h2 class="font-bold text-2xl sm:text-3xl pt-10">Daftar Alat & Barang</h2>

					<div class="flex items-center justify-between gap-4 flex-wrap w-full">

						<form action="{{ route('admin-alat') }}#search-alat" method="GET"
							...
							<div class="relative flex-1 w-full h-full flex-shrink-0">
								...
								<input id="search-alat" type="text" name="search" placeholder="Cari nama alat..."
									...
									<div class="py-2">
										...
										<label
											...
											<input type="radio" name="kategori_id" value="" onchange="this.form.submit()"
												class="mr-3 w-4 h-4 text-[#363062] focus:ring-[#363062]" {{ request('kategori_id') == '' ? 'checked' : '' }}>
											Semua Kategori
										</label>
										...
										@foreach($kategori as $kat)
											<label
												...
												<input type="radio" name="kategori_id" value="{{ $kat->id }}"
													onchange="this.form.submit()"
													class="mr-3 w-4 h-4 text-[#363062] focus:ring-[#363062]" {{ request('kategori_id') == $kat->id ? 'checked' : '' }}>
												{{ $kat->nama_kategori }}
											</label>
										@endforeach
										...
										<label
											class="flex items-center px-4 py-2.5 text-sm text-[#363062] hover:bg-gray-50 cursor-pointer transition">
											<input type="radio" name="durasi" value="" onchange="this.form.submit()"
												class="mr-3 w-4 h-4 text-[#363062] focus:ring-[#363062]" {{ request('durasi') == '' ? 'checked' : '' }}>
											Semua Durasi
										</label>
										...
										@foreach($alat->pluck('durasi')->unique()->filter()->sort() as $durasi)
											<label
												class="flex items-center px-4 py-2.5 text-sm text-[#363062] hover:bg-gray-50 cursor-pointer transition">
												<input type="radio" name="durasi" value="{{ $durasi }}"
													onchange="this.form.submit()"
													class="mr-3 w-4 h-4 text-[#363062] focus:ring-[#363062]" {{ request('durasi') == $durasi ? 'checked' : '' }}>
												{{ $durasi }} Menit
											</label>
										@endforeach
									</div>
					...
					<div class="w-full grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
						@forelse ($alat as $data)
							<div class="w-full h-[320px] sm:h-[360px] lg:h-[400px]">
										...
										<div class="backdrop-blur-md bg-white/20 p-4 text-white">
											...
											<div class="flex items-center justify-between">
												...
												<div class="flex flex-col">
													<p class="text-base sm:text-lg font-semibold line-clamp-1">
														{{ $data->nama_alat }}
													</p>

													<span class="text-xs sm:text-sm font-medium">Kategori:
														{{ $data->kategori->nama_kategori }}</span>

												</div>

												<div class="flex flex-col gap-2 items-center">
													@if(($data->dipinjam_count ?? 0) > 0)
														<div class="flex items-center h-7 sm:h-8 w-24 sm:w-32 gap-2">
															<div
																class="h-full w-full flex items-center justify-center gap-2 bg-white border border-[#F99417] rounded-full p-2">
																<span class="w-2 h-2 rounded-full bg-[#F99417]"></span>
																<span
																	class="text-[11px] sm:text-sm text-[#F99417]">{{ $data->dipinjam_count ?? 0 }}
																	Dipinjam</span>
															</div>
														</div>
													@endif
													...
													<div class="flex items-center h-7 sm:h-8 w-24 sm:w-32 gap-2">
														@if ($data->stok <= 7)
															<div
																class="h-full w-full flex items-center justify-center gap-2 border border-white bg-white/70 rounded-full p-2">
																<span class="w-2 h-2 rounded-full bg-red-500"></span>
																<span
																	class="text-[11px] sm:text-sm font-semibold text-red-500">{{ $data->stok }}
																	Tersisa</span>
															</div>
														@else
															<div
																class="h-full w-full flex items-center justify-center gap-2 border border-white rounded-full p-2">
																<span class="w-2 h-2 rounded-full bg-white"></span>
																<span class="text-[11px] sm:text-sm">{{ $data->stok }}
																	Tersisa</span>
															</div>
														@endif
													</div>
											...
											<div class="flex items-center justify-between mt-2">
												...
												<div class="h-8 w-28 sm:w-32 flex items-center gap-1 sm:gap-2">
													<a href="{{ route('admin-alat-edit', $data) }}"
														class="h-full w-full inline-flex items-center justify-center rounded-full bg-white text-[#F99417] text-xs sm:text-sm hover:bg-[#F99417] hover:text-white border border-[#F99417] hover:border-transparent transition">
														Edit
													</a>
													<form action="{{ route('admin-alat-destroy', $data) }}" method="POST"
														class="h-full w-full">
														...
														@method('DELETE')
														<button
															class="h-full w-full rounded-full bg-white text-red-500 text-xs sm:text-sm hover:bg-red-500 hover:text-white border border-red-500 hover:border-transparent transition">
															Hapus
														</button>
													</form>
												</div>
						...
						@empty
							<div class="col-span-3 py-4 flex flex-col gap-2 items-center justify-center">
								...
								<span class="text-sm text-gray-500">Anda masih belum menambahkan data alat & barang
									peminjaman...</span>
							</div>
						@endforelse
					</div>
					...
					@if($alat->hasPages())
						<div class="mt-6">
							<div class="flex flex-col sm:flex-row items-center justify-between gap-4">
								...
								{{-- Keterangan Data --}}
								<div class="text-sm text-gray-500 order-2 sm:order-1 text-center sm:text-left">
									Menampilkan {{ $alat->firstItem() }} sampai {{ $alat->lastItem() }} dari
									{{ $alat->total() }} alat
								</div>
								...
								<div class="flex items-center gap-2 order-1 sm:order-2">
									...
									@if($alat->onFirstPage())
										<button type="button" disabled
											...
											<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 rotate-180" viewBox="0 0 12 24">
												<defs>
													<path id="alat_prev" fill="currentColor"
														d="m7.588 12.43l-1.061 1.06L.748 7.713a.996.996 0 0 1 0-1.413L6.527.52l1.06 1.06l-5.424 5.425z" />
												</defs>
												<use fill-rule="evenodd" href="#alat_prev" transform="rotate(-180 5.02 9.505)" />
											</svg>
										</button>
									@else
										<a href="{{ $alat->previousPageUrl() }}"
											...
											<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 rotate-180" viewBox="0 0 12 24">
												<defs>
													<path id="alat_prev2" fill="currentColor"
														d="m7.588 12.43l-1.061 1.06L.748 7.713a.996.996 0 0 1 0-1.413L6.527.52l1.06 1.06l-5.424 5.425z" />
												</defs>
												<use fill-rule="evenodd" href="#alat_prev2" transform="rotate(-180 5.02 9.505)" />
											</svg>
										</a>
									@endif
									...
									<div class="flex items-center gap-1 bg-gray-100 p-1.5 sm:p-2 rounded-full shadow-inner">
										@php
											...
											if ($start > 1) {
												...
												echo '<a href="' . $alat->url(1) . '" class="w-8 h-8 flex items-center justify-center rounded-full text-sm font-medium transition ' . $activeClassFirst . '">1</a>';
											...
											for ($i = $start; $i <= $end; $i++) {
												...
												echo '<a href="' . $alat->url($i) . '" class="w-8 h-8 flex items-center justify-center rounded-full text-sm font-medium transition ' . $activeClass . '">' . $i . '</a>';
											}
											...
											if ($end < $lastPage) {
												...
												echo '<a href="' . $alat->url($lastPage) . '" class="w-8 h-8 flex items-center justify-center rounded-full text-sm font-medium transition ' . $activeClassLast . '">' . $lastPage . '</a>';
											}
										@endphp
									</div>
									...
									@if($alat->hasMorePages())
										<a href="{{ $alat->nextPageUrl() }}"
											...
											<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 12 24">
												<defs>
													<path id="alat_next" fill="currentColor"
														d="m7.588 12.43l-1.061 1.06L.748 7.713a.996.996 0 0 1 0-1.413L6.527.52l1.06 1.06l-5.424 5.425z" />
												</defs>
												<use fill-rule="evenodd" href="#alat_next" transform="rotate(-180 5.02 9.505)" />
											</svg>
										</a>
									@else
										<button type="button" disabled
											...
											<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 12 24">
												<defs>
													<path id="alat_next2" fill="currentColor"
														d="m7.588 12.43l-1.061 1.06L.748 7.713a.996.996 0 0 1 0-1.413L6.527.52l1.06 1.06l-5.424 5.425z" />
												</defs>
												<use fill-rule="evenodd" href="#alat_next2" transform="rotate(-180 5.02 9.505)" />
											</svg>
										</button>
									@endif
